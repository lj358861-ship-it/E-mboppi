...
export default function AvisProduit({ produitId }: { produitId: string }) {
  ...
  function charger() {
    fetch(`/api/avis-produit?produitId=${produitId}`)
  ...
  async function envoyerAvis(e: React.FormEvent) {
    ...
    setEnvoi(true);
    try {
      await fetch("/api/avis-produit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ produitId, note: noteChoisie, commentaire }),
      });
  ...
  return (
    <section className="mt-8 bg-white border border-stone-200 rounded-2xl p-5">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        <div>
          <p className="font-display text-lg font-semibold text-indigo-900">Avis sur cet article</p>
          {nbAvis > 0 ? (
